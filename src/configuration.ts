  ...
  static fromRunnableConfig(config?: RunnableConfig): Configuration {
    ...
    for (const fieldName of fieldNames) {
      ...
      if (value !== undefined && value !== null) {
        ...
        if (fieldName.includes('_retries') || fieldName.includes('_iterations') || fieldName.includes('_calls') || fieldName.includes('_units') || fieldName.includes('_tokens')) {
          values[fieldName] = typeof value === 'string' ? parseInt(value, 10) : value;
        } else if (fieldName === 'allow_clarification') {
          values[fieldName] = typeof value === 'string' ? value.toLowerCase() === 'true' : value;
        } else {
