...
export async function clarifyWithUser(
  ...
  const model = configurableModel
    ...
    .withRetry({ stopAfterAttempt: configurable.max_structured_output_retries })
    .bind(modelConfig);
      ...
      value: [
        new SystemMessage({
          content: leadResearcherPrompt
            .replace("{date}", getTodayStr())
            .replace("{max_concurrent_research_units}", configurable.max_concurrent_research_units.toString())
        }),
...
export async function supervisorTools(
  ...
  const mostRecentMessage = supervisorMessages[supervisorMessages.length - 1] as AIMessage;
  
  // Exit Criteria
  const exceededAllowedIterations = researchIterations >= configurable.max_researcher_iterations;
  const noToolCalls = !mostRecentMessage.tool_calls || mostRecentMessage.tool_calls.length === 0;
  ...
  try {
    const allConductResearchCalls = mostRecentMessage.tool_calls?.filter(
      toolCall => toolCall.name === "ConductResearch"
    ) || [];
    
    const conductResearchCalls = allConductResearchCalls.slice(0, configurable.max_concurrent_research_units);
    const overflowConductResearchCalls = allConductResearchCalls.slice(configurable.max_concurrent_research_units);
    
    const researchPromises = conductResearchCalls.map(async (toolCall) => {
      return researcherSubgraph.invoke({
        researcher_messages: [
          new HumanMessage({ content: toolCall.args.research_topic })
        ],
    ...
    toolResults.forEach((observation, index) => {
      ...
      toolMessages.push(new ToolMessage({
        content: observation.compressed_research || "Error synthesizing research report: Maximum retries exceeded",
        name: toolCall.name,
...
export async function researcher(
  ...
  if (tools.length === 0) {
    throw new Error("No tools found to conduct research: Please configure either your search API or add MCP tools to your configuration.");
  }
...
export async function researcherTools(
  ...
  const configurable = Configuration.fromRunnableConfig(config);
  const researcherMessages = state.researcher_messages;
  const mostRecentMessage = researcherMessages[researcherMessages.length - 1] as AIMessage;
  
  // Early Exit Criteria: No tool calls were made by the researcher
  if (!mostRecentMessage.tool_calls || mostRecentMessage.tool_calls.length === 0) {
  ...
  // Late Exit Criteria
  const exceededToolCallIterations = (state.tool_call_iterations || 0) >= configurable.max_react_tool_calls;
  const researchCompleteToolCall = toolCalls.some(toolCall => toolCall.name === "ResearchComplete");
  ...
  while (synthesisAttempts < 3) {
    try {
      const response = await synthesizerModel.invoke([
        new SystemMessage({ 
          content: compressResearchSystemPrompt.replace("{date}", getTodayStr()) 
        }),
        ...researcherMessages
      ]);
      
      const filteredMessages = researcherMessages.filter(msg => 
        msg._getType() === "tool" || msg._getType() === "ai"
      );
      ...
      if (isTokenLimitExceeded(error as Error, configurable.research_model)) {
        ...
        console.error(`Token limit exceeded while synthesizing: ${error}. Pruning the messages to try again.`);
        continue;
      }
...
export async function finalReportGeneration(
  ...
  while (currentRetry <= maxRetries) {
    ...
    try {
      ...
      if (isTokenLimitExceeded(error as Error, configurable.final_report_model)) {
        if (currentRetry === 0) {
          const modelTokenLimit = getModelTokenLimit(configurable.final_report_model);
          if (!modelTokenLimit) {
            return {
...
const researcherBuilder = new StateGraph<ResearcherState, ResearcherOutputState>({
  channels: {
    researcher_messages: { value: (current: BaseMessage[], update: BaseMessage[]) => [...current, ...update], default: () => [] },
    tool_call_iterations: { value: (current: number, update: number) => update, default: () => 0 },
    research_topic: { value: (current: string, update: string) => update, default: () => "" },
    compressed_research: { value: (current: string, update: string) => update, default: () => "" },
    raw_notes: { value: (current: string[], update: any) => {
      if (typeof update === "object" && update !== null && "type" in update && update.type === "override") {
        return update.value;
      } else {
...
const supervisorBuilder = new StateGraph<SupervisorState>({
  channels: {
    supervisor_messages: { value: (current: BaseMessage[], update: any) => {
      if (typeof update === "object" && update !== null && "type" in update && update.type === "override") {
        return update.value;
      } else {
        return [...current, ...update];
    ...
    research_brief: { value: (current: string, update: string) => update, default: () => "" },
    notes: { value: (current: string[], update: any) => {
      if (typeof update === "object" && update !== null && "type" in update && update.type === "override") {
        return update.value;
      } else {
        return [...current, ...update];
    ...
    research_iterations: { value: (current: number, update: number) => update, default: () => 0 },
    raw_notes: { value: (current: string[], update: any) => {
      if (typeof update === "object" && update !== null && "type" in update && update.type === "override") {
        return update.value;
      } else {
...
const deepResearcherBuilder = new StateGraph<AgentState, AgentInputState>({
  channels: {
    messages: { value: (current: BaseMessage[], update: BaseMessage[]) => [...current, ...update], default: () => [] },
    supervisor_messages: { value: (current: BaseMessage[], update: any) => {
      if (typeof update === "object" && update !== null && "type" in update && update.type === "override") {
        return update.value;
      } else {
    ...
    final_report: { value: (current: string, update: string) => update, default: () => "" }
  }
});
